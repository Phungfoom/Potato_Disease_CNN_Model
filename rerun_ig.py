"""Rerun Integrated Gradients on a saved model without retraining.

Usage:
    # First N images from val set (default):
    python rerun_ig.py --model outputs/fusion/models/potato_leaf_model_20260408_002451.keras

    # 3 images per class (recommended for reports):
    python rerun_ig.py --model outputs/fusion/models/potato_leaf_model_20260408_002451.keras --per-class 3

    # Specific images by val-set index (cohesive with Grad-CAM/occlusion):
    python rerun_ig.py --model outputs/fusion/models/potato_leaf_model_20260408_002451.keras --indices 3 17 42 88 105
"""
...
from grad_cam_visualizer import ensure_grad_targets
...
parser.add_argument(
    ...
    help="Number of interpolation steps along the integration path.",
)
parser.add_argument(
    "--target",
    default="predicted",
...
parser.add_argument(
    "--no-gradcam-compare",
    action="store_true",
    help="Skip side-by-side Grad-CAM comparison panel.",
)
parser.add_argument(
    "--base-dir",
    type=str,
...
def _pick_per_class_indices(n_per_class):
    ...
    print("To reuse these exact images across Grad-CAM/occlusion, run with:")
...
csv_path = os.path.join(metrics_dir, f"ig_summary_{timestamp}.csv")

print(f"Saving Integrated Gradients outputs to: {save_dir}")
run_integrated_gradients_batch(
    model=model,
    ...
    m_steps=args.steps,
    baseline="black",
    ig_target=args.target,
    csv_path=csv_path,
    compare_gradcam=not args.no_gradcam_compare,
)
